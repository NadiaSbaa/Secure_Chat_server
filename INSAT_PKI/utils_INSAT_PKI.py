...
def createCA():
    ...
    open('INSAT_PKI.pkey', 'wb').write(crypto.dump_privatekey(crypto.FILETYPE_PEM, cakey))
    open('INSAT_PKI.cert', 'wb').write(crypto.dump_certificate(crypto.FILETYPE_PEM, cacert))
...
def createRequest(origin):
    ...
    pkey_file_name = '../Client/keys/' + str(origin) + '.pkey'
    open(pkey_file_name, 'wb').write(crypto.dump_privatekey(crypto.FILETYPE_PEM, pkey))        
    return pkey, req
